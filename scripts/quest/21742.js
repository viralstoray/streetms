...
function start(mode, type, selection) {
	...
	if (status == 0) {
		qm.sendNext("Well, I'm not really busy or anything, but I don't feel like concocting medicine. Can you come back later? If you don't mind, move.", 9);
	} else if (status == 1) {
		qm.sendNextPrev("I heard you met the Shadow Knight of the Black Wings...", 3);
	} else if (status == 2) {
		qm.sendNextPrev("Ah, you mean that guy dressed in black with a menacing wrinkle in his forehead? Why, yes I did. I did meet him. I even got an item for him. He asked me to deliver it to that old man, Mu Gong.", 9);
	} else if (status == 3) {
		qm.sendNextPrev("An item?", 3);
	} else if (status == 4) {
		qm.sendNextPrev("Yes, a big #bHanging Scroll#k. He gave it to me without saying much. He just asked me to deliver it. He looked scary, as if he would chase me down if I didn't do as he said. Wheeeeeeeew, that was an experience.", 9);
	} else if (status == 5) {
		qm.sendNextPrev("So did you deliever the Hanging Scroll to him?", 3);
	} else if (status == 6) {
