...
function end(mode, type, selection) {
	...
	if (status == 0) {
		...
		qm.changeJobById(1510);
		qm.gainItem(1142067, 1);
		qm.removeAll(4032010);
		qm.sendNext("You are a Knight-in-Training no more. You are now an official Knight of Cygnus.");
	} else if (status == 2) {
		qm.sendNext("I have given you some #bSP#k. I have also given you a number of skills for a Thunder Breaker that's only available to knights, so I want you to work on it and hopefully cultivate it as much as your soul.");
	} else if (status == 3) {
