...
function start(mode, type, selection) {
	...
	if (status == 0) {
		...
		qm.sendPrev("#b(He has mistaken you for a hero's successor. But what does he mean when he mentions continuing the legacy of the heroes? You'll have to stop the Shadow Knight first before asking him.)#k", 3);
		qm.dispose();
	}
}
...
function end(mode, type, selection) {
	...
	if (status == 0) {
		...
		qm.sendNext("I see. So you lost the Seal Stone of Mu Lung... That's unfortunate, but there is nothing you can do about it. I, too, do not have a clue why the heroes left the Seal Stone in Mu Lung.", 9);
	} else if (status == 2) {
		qm.sendNext("Are you sure the heroes left the Seal Stone in Mu Lung?", 3);
	} else if (status == 3) {
		qm.sendNextPrev("Yes, I suppose you weren't aware of that. #bA long, long time ago, the heroes left the Seal Stone in Mu Lung, and the chief made the Sealed Temple to keep it safe#k.", 9);
	} else if (status == 4) {
		qm.sendNextPrev("The heroes...", 3);
	} else if (status == 5) {
		qm.sendNextPrev("Hardly anyone knows the existence of something like that nowadays. Honestly, #bI'm not even sure that losing the Seal Stone will have any negative effects on Mu Lung#k. We just valued it since it was something the heroes left us for safe keeping.", 9);
	} else if (status == 6) {
		...
		qm.sendPrev("#b(Mu Lung has been lost... You better consult with Tru.)#k", 3);
		qm.dispose();
	}
}